...
export default function CharacterInfo(props) {
    ...
    return html`
        ...
        <div class=section-content style='text-align: center;'>${props.currentCharacter.meaning}</div>
        ...
        <div class=section-title><a onclick=${ev => ev.stopPropagation()} style='color: white; text-decoration: underline;' href='#database/expressions/?${props.currentCharacter.hanzi}'>Expressions</a></div>
        <ul style='background-color: transparent; padding: 0; margin-top: 0; margin-bottom: 0;' class='section-content expandable'>
            ...
            <li style='display: flex; gap: 0.2em; margin: 0; padding: 0;'>
                <a style='color: white; white-space: nowrap;' href='#' onclick=${ev => { ev.stopPropagation(); ev.preventDefault(); setSelectedShortInfo(e); }}>${e.hanzi}</a>
                <span class='elipsible'>(${e.pinyin})</span>
            </li>
            `) : '(none yet)'}
        </ul>
        <div class=section-title>Don't confuse with ${!isEditingRelated ? html`<button onclick=${e => { e.stopPropagation(); setEditingRelated(true); }}>✎</button>` : ''}</div>
        <div class=section-content>
            ${props.currentCharacter.related.length > 0 ? props.currentCharacter.related.map(c => html`
            <a style='color: white;' href='#' onclick=${ev => { ev.stopPropagation(); ev.preventDefault(); setSelectedShortInfo(c); }}>${c.hanzi}</a>
            `) : '(none yet)'}
        </div>
